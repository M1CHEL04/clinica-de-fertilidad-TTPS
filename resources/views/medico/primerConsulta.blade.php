...
<form action="{{ $tratamiento
    ? route('consulta.update', $tratamiento->id)
    ...
    @endif
        ...
        <div class="mb-3">
            ...
            <div class="dropdown">
                <input type="text"
                       id="antecedente-input"
                       class="form-control"
                       placeholder="Ingresar 3 letras..." >

                <div id="antecedente-dropdown"
                     class="dropdown-menu p-2"
                     style="max-height: 180px; overflow-y: auto;">
                </div>
            </div>
            @php
                $antecedentes = json_decode($tratamiento?->antecedentesPersonales?->antecedentes, true) ?? [];
            @endphp
        ...
        <div class="mb-3">
            ...
            <select name="fuma" id="fuma" class="form-select" required>
                ...
                <option value="si" {{ $tratamiento?->antecedentesPersonales?->fuma > 0 ? 'selected' : '' }}>Sí</option>
                <option value="no" {{ $tratamiento?->antecedentesPersonales?->fuma === 0 ? 'selected' : '' }}>No</option>
            </select>
        </div>
        ...
        <div class="mb-3" id="campo-cantidad" style="{{ $tratamiento?->antecedentesPersonales?->fuma > 0 ? '' : 'display:none;' }}">
            ...
            <div class="row">
                <div class="col-md-4">
                    <input type="number"
                           ...
                           value="{{ old('cant_cigarros', $detalleCant[0]) }}">
                </div>
                ...
                <div class="col-md-4">
                    <input type="number"
                           ...
                           value="{{ old('dias_fuma', $detalleCant[1]) }}">
                </div>
                ...
                <div class="col-md-4">
                    <input type="number"
                           ...
                           value="{{ old('anios_fuma', $detalleCant[2]) }}">
                </div>
                @if($tratamiento?->antecedentesPersonales?->cuanto_fuma > 0)
                    <p>Resultado anteriormente almacenado segun el estandar: {{$tratamiento?->antecedentesPersonales?->cuanto_fuma}}</p>
                @endif
            </div>
        ...
        <div class="mb-3">
            ...
            <select name="alcohol" id="alcohol" class="form-select" required>
                <option value="">Seleccione...</option>
                <option value="si" {{ $tratamiento?->antecedentesPersonales?->alcohol > 0 ? 'selected' : '' }}>Sí</option>
                <option value="no" {{ $tratamiento?->antecedentesPersonales?->alcohol === 0 ? 'selected' : '' }}>No</option>
            </select>
        </div>

        <div id="alcohol-extra" style="{{ $tratamiento?->antecedentesPersonales?->alcohol > 0 ? '' : 'display:none;' }}">
        ...
        <div class="mb-3">
            ...
            <select name="droga" id="droga" class="form-select" required>
                <option value="">Seleccione...</option>
                <option value="si" {{ $tratamiento?->antecedentesPersonales->droga > 0 ? 'selected' : '' }}>Sí</option>
                <option value="no" {{ $tratamiento?->antecedentesPersonales->droga === 0 ? 'selected' : '' }}>No</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="observaciones"
                      id="observaciones"
                      class="form-control"
            >{{ old('observaciones', $tratamiento?->antecedentesPersonales?->observaciones) }}</textarea>
        </div>
        ...
        @if($tratamiento?->antecedentesFamiliares)
            @foreach($tratamiento->antecedentesFamiliares as $index => $familiar)
                ...
                <div class="mb-3 familiar-entry" data-index="{{ $index }}">
                    ...
                    <input type="text" 
                           name="patologias_familiares[{{ $index }}]" 
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <select name="ciclos" id="ciclos" class="form-select" required>
                    <option value="">Seleccione...</option>
                    <option value="regular" {{ ($g?->ciclo_regular ?? '')=== 1 ? 'selected' : '' }}>Regular</option>
                    <option value="irregular" {{ ($g?->ciclo_regular ?? '')=== 0 ? 'selected' : '' }}>Irregular</option>
                </select>
            </div>
            <div class="col-md-4">
                <label for="duracion" class="form-label required">Duración del ciclo</label>
                <input type="number" name="duracion" id="duracion" class="form-control" placeholder="Ej: 21 dias" value="{{ old('duracion', $g?->duracion) }}" required>
            </div>
            <div class="col-md-4">
                <label for="caracteristica" class="form-label required">Característica del sangrado</label>
                <input type="text" name="caracteristica" id="caracteristica" class="form-control" placeholder="Ej: abundante, leve" value="{{ old('caracteristica', $g?->caracteristicas_sangrado) }}" required>
            </div>
            <div class="col-md-4 mt-3">
                <label for="menarca" class="form-label required">Menarca (edad)</label>
                <input type="number" name="menarca" id="menarca" class="form-control" placeholder="Ej: 12 años" value="{{ old('menarca', $g?->edad_menarca) }}" required>
            </div>
            <div class="col-md-3 mt-3">
                <label for="embarazos" class="form-label required">G</label>
                <input type="number" name="embarazos" id="embarazos" class="form-control" placeholder="Cantidad de embarazos" value="{{ old('embarazos', $g?->G) }}" required>
            </div>
            <div class="col-md-3 mt-3">
                <label for="partos" class="form-label required">P</label>
                <input type="number" name="partos" id="partos" class="form-control" placeholder="Cantidad de partos" value="{{ old('partos', $g?->P) }}" required>
            </div>
            <div class="col-md-3 mt-3">
                <label for="abortos" class="form-label required">AB</label>
                <input type="number" name="abortos" id="abortos" class="form-control" placeholder="Cantidad de abortos" value="{{ old('abortos', $g?->AB) }}" required>
            </div>
            <div class="col-md-3 mt-3">
                <label for="ectopicos" class="form-label required">CT</label>
                <input type="number" name="ectopicos" id="ectopicos" class="form-control" placeholder="Cantidad de embarazos ectópicos" value="{{ old('ectopicos', $g?->CT) }}" required>
            </div>
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <select name="color-ojos" id="color-ojos" class="form-select" required>
                    <option value="">Seleccione...</option>
                    <option value="ambar" {{ ($f?->color_ojos ?? '')==='ambar' ? 'selected' : '' }}>Ambar</option>
                    <option value="castaño" {{ ($f?->color_ojos ?? '')==='castaño' ? 'selected' : '' }}>Castaño</option>
                    <option value="avellana" {{ ($f?->color_ojos ?? '')==='avellana' ? 'selected' : '' }}>Avellana</option>
                    <option value="azul" {{ ($f?->color_ojos ?? '')==='azul' ? 'selected' : '' }}>Azul</option>
                    <option value="verde" {{ ($f?->color_ojos ?? '')==='verde' ? 'selected' : '' }}>Verde</option>
                    <option value="gris" {{ ($f?->color_ojos ?? '')==='gris' ? 'selected' : '' }}>Gris</option>
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="color-pelo" id="color-pelo" class="form-select" required>
                    <option value="">Seleccione...</option>
                    <option value="negro" {{ ($f?->color_pelo ?? '')==='negro' ? 'selected' : '' }}>Negro</option>
                    <option value="castaño" {{ ($f?->color_pelo ?? '')==='castaño' ? 'selected' : '' }}>Castaño</option>
                    <option value="rubio" {{ ($f?->color_pelo ?? '')==='rubio' ? 'selected' : '' }}>Rubio</option>
                    <option value="pelirrojo" {{ ($f?->color_pelo ?? '')==='pelirrojo' ? 'selected' : '' }}>Pelirrojo</option>
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="tipo-pelo" id="tipo-pelo" class="form-select" required>
                    <option value="">Seleccione...</option>
                    <option value="rizado" {{ ($f?->tipo_pelo ?? '')==='rizado' ? 'selected' : '' }}>Rizado</option>
                    <option value="liso" {{ ($f?->tipo_pelo ?? '')==='liso' ? 'selected' : '' }}>Liso</option>
                    <option value="ondulado" {{ ($f?->tipo_pelo ?? '')==='ondulado' ? 'selected' : '' }}>Ondulado</option>
                </select>
            </div>
            <div class="col-md-4 mt-3">
                <label for="altura" class="form-label required">Altura (cm)</label>
                <input type="number" name="altura" id="altura" class="form-control" placeholder="Ej: 172" value="{{$f?->altura}}" required>
            </div>
            <div class="col-md-4 mt-3">
                <label for="complexion" class="form-label required">Complexión</label>
                <input type="text" name="complexion" id="complexion" class="form-control" placeholder="Ej: delgada, robusta" value="{{$f?->complexion_corporal}}" required>
            </div>
            <div class="col-md-4 mt-3">
                <label for="rasgos-etnicos" class="form-label required">Rasgos étnicos</label>
                <input type="text" name="rasgos-etnicos" id="rasgos-etnicos" class="form-control" placeholder="Ej: asiático" value="{{ $f?->rasgos_etnicos }}" required>
            </div>
        </div>
