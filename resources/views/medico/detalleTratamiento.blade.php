...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <span
                            class="badge 
                        {{ strtolower($tratamiento->estado_tratamiento) === 'activo'
                            ? 'badge-success'
                            : (strtolower($tratamiento->estado_tratamiento) === 'completado'
                                ? 'badge-info'
                                : 'badge-warning') }}">
            ...
            @endphp
            @php
                $etapaOperador = in_array($etapa, ['puncion', 'fertilizacion']);
            @endphp
            ...
            <div class="card p-6">
                ...
                @if (session('rol') == 2 || session('rol') == 5)
                            ...
                            <form method="POST" action="{{ route('tratamiento.retroceder-etapa', $tratamiento->id) }}">
                                ...
                                @if ($etapaOperador)
                                    <button class="btn-secondary opacity-50 cursor-not-allowed" disabled>
                                        <i class="fas fa-arrow-left mr-2"></i> Retroceder etapa
                                    </button>
                                @else
                            ...
                            <form method="POST" action="{{ route('tratamiento.avanzar-etapa', $tratamiento->id) }}">
                                ...
                                @if ($etapaOperador)
                                    <button class="btn-primary opacity-50 cursor-not-allowed" disabled>
                                        <i class="fas fa-arrow-right mr-2"></i> Avanzar etapa
                                    </button>
                                @else
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <button class="btn-primary {{ $deshabilitado ? 'opacity-50 cursor-not-allowed' : '' }}"
                        ...
                        <form method="POST" action="{{ route('tratamiento.notificar-transferencia', $tratamiento->id) }}"
                            ...
                            @csrf
                            <button type="submit"
                                class="btn-primary {{ $habilitado ? '' : 'opacity-50 cursor-not-allowed' }} w-full"
                                {{ $habilitado ? '' : 'disabled' }}>
                                <i class="fas fa-bell mr-2"></i> Notificar transferencia
                            </button>
                        </form>
                    </div>
                        ...
                        <div class="modal-content">
                            ...
                            <form method="POST" action="{{ route('tratamiento.agendar-consulta', $tratamiento->id) }}">
                                ...
                                <div class="modal-actions">
                                    ...
                                    <button type="submit" class="btn-primary"><i class="fas fa-save me-2">
                                            Guardar</i></button>
                                </div>
                            </form>
                        </div>
            ...
            @if (session('rol') == 3)
                ...
                <button class="btn-primary opacity-50 cursor-not-allowed w-full mb-4" disabled>
                    <i class="fas fa-calendar-plus mr-2"></i> Agendar nueva consulta
                </button>
                        ...
                        <form method="POST" action="{{ route('tratamiento.agendar-consulta', $tratamiento->id) }}">
                            ...
                            <div class="modal-actions">
                                ...
                                <button type="submit" class="btn-primary"> <i
                                        class="fas fa-save me-2">Guardar</i></button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    {{-- CONTROL DE EMBARAZO --}}
    @php
        $etapa4 = in_array($etapa, ['control de embarazo', 'finalizado']);
    @endphp
    <!-- 🧰 Panel lateral -->
        ...
        <div class="space-y-4">
            <div class="card p-6">
                ...
                @if (session('rol') == 5)
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('jefe.verConsulta', $tratamiento->paciente_id) }}"
                        ...
                        @if ($etapa1)
                            <a href="{{ route('jefe.estudios.index', $tratamiento->paciente_id) }}"
                                class="btn-primary w-full flex items-center justify-center gap-2">
                        ...
                        @if ($etapa2)
                            <a href="{{ route('jefe.tratamiento.cargar-estudios', $tratamiento->id) }}"
                                class="btn-primary w-full flex items-center justify-center gap-2">
                        ...
                        @if ($etapa2)
                            <a href="{{ route('jefe.tratamiento.protocolo', $tratamiento->id) }}"
                                class="btn-primary w-full flex items-center justify-center gap-2">
                        ...
                        @if ($etapa3)
                            <a href="{{ route('jefe.monitoreos', $tratamiento->id) }}"
                                class="btn-primary w-full flex items-center justify-center gap-2">
            ...
            @elseif (session('rol') == 3)
                <div class="space-y-3">
                    ...
                    <a href="{{ route('operador.verConsulta', $tratamiento->paciente_id) }}"
                        class="btn-primary w-full flex items-center justify-center gap-2"
                        {{ !$etapa1 ? 'disabled' : '' }}>
                        <i class="fas fa-bullseye"></i> Seccion de primera consulta
                    </a>
...
@else
    <div class="space-y-3">
        ...
        <a href="{{ route('medico.verConsulta', $tratamiento->paciente_id) }}"
            class="btn-primary w-full flex items-center justify-center gap-2" {{ !$etapa1 ? 'disabled' : '' }}>
            <i class="fas fa-bullseye"></i> Seccion de primera consulta
        </a>
