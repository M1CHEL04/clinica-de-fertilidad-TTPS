    ...
    <div class="lg:col-span-2 space-y-6">
                    ...
                    <div x-show="detail" x-transition class="p-3 bg-gray-50 border-t text-gray-700">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p><strong>Operador:</strong> {{ $fertilizacion->operador->nombre ?? 'N/A' }} {{ $fertilizacion->operador->apellido ?? '' }}</p>
                            </div>
                            <div>
        ...
        <div class="card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="border rounded-lg p-5 bg-gradient-to-r from-purple-50 to-blue-50 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900 text-base mb-1">{{ $embrion->identificador }}</h4>
                            <p class="text-sm text-gray-600">FERT_{{ $embrion->fertilizacion->id }}</p>
                        </div>
                        @php
                    ...
                    <div class="flex gap-2 mt-4">
                        <button class="flex-1 text-xs px-3 py-1.5 bg-white border border-blue-200 text-blue-700 rounded-md hover:bg-blue-50 transition-colors flex items-center justify-center gap-1"
                        ...
                        <div class="flex-1 text-xs px-3 py-1.5 {{ $estadoClase }} rounded-md flex items-center justify-center gap-1">
...
<div id="modalCambioEstado" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden" onclick="cerrarModalCambio()">
    <div class="flex items-start justify-center min-h-screen p-4 pt-20">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[calc(100vh-8rem)] overflow-y-auto" onclick="event.stopPropagation()">
            ...
            <form id="formCambioEstado" method="POST" action="{{ route('embrion.update') }}" class="p-6">
                ...
                <div class="space-y-6">
                    <div class="relative">
                        ...
                        <select name="nueva_accion" id="nuevaAccion" class="w-full border border-gray-300 rounded-lg px-3 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent appearance-none bg-white" required onchange="toggleMotivoDescarte()">
                    ...
                    <div id="motivoDescarteDiv" class="hidden">
                        ...
                        <textarea name="motivo_descarte" id="motivoDescarte" class="w-full border border-red-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent resize-none" rows="4" placeholder="Especifique el motivo del descarte..."></textarea>
                    </div>
                </div>
                
                <div class="flex justify-end gap-3 mt-8 pt-4 border-t">
                    <button type="button" onclick="cerrarModalCambio()" class="px-4 py-2 text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
        ...
        const compatibilidadInfo = embrion.compatibilidad_gameto ? 
            `<div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                ...
                <p class="text-sm text-green-700">
                    <strong>Compatibilidad:</strong> ${(embrion.compatibilidad_gameto * 100)}%
                </p>
            </div>` : '';
