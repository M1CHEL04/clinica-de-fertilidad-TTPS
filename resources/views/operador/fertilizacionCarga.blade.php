        ...
        <div class="card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="date" class="form-input" name="fecha_fertilizacion" id="fechaFertilizacion" required>
                </div>

                <!-- Hora -->
                <div>
                    <label class="form-label">Hora *</label>
                    <input type="time" class="form-input" name="hora_fertilizacion" id="horaFertilizacion" required>
                </div>
                        ...
                        <div class="border rounded-lg p-4 bg-gradient-to-r from-yellow-50 to-orange-50">
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex items-center">
                                    <input type="checkbox" id="ovocito_{{ $ovocito->id }}" name="ovocitos_seleccionados[]"
                                        value="{{ $ovocito->id }}" class="mr-3 ovocito-checkbox"
                            ...
                            <div class="text-sm text-gray-600">
                                <p><strong>Calidad morfológica:</strong>
                                    {{ $ovocito->calidad_morfologica ?? 'Sin evaluar' }}</p>
                            </div>
                        </div>
                    @endforeach
                @else
...
@section('scripts')
        ...
        <div class="border rounded-lg p-0 bg-white shadow-sm" id="embrion_${ovocitoId}">
            ...
            <div class="p-4 space-y-4">
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center p-2 bg-gray-50 rounded border cursor-pointer hover:bg-gray-100">
                            <input type="radio" name="embriones[${ovocitoId}][fuente_semen]" value="fresco" 
                                   class="mr-3 fuente-semen-radio" data-ovocito="${ovocitoId}" required>
                            <span class="text-sm text-gray-700">Semen de la pareja (En fresco)</span>
                        </label>
                        <label class="flex items-center p-2 bg-gray-50 rounded border cursor-pointer hover:bg-gray-100">
                            <input type="radio" name="embriones[${ovocitoId}][fuente_semen]" value="pareja" 
                                   class="mr-3 fuente-semen-radio" data-ovocito="${ovocitoId}" required>
                            <span class="text-sm text-gray-700">Semen de la pareja (Criopreservado)</span>
                        </label>
                        <label class="flex items-center p-2 bg-gray-50 rounded border cursor-pointer hover:bg-gray-100">
    ...
    `;
        ...
        function manejarCambioFuenteSemen(ovocitoId, tipoFuente) {
            ...
            } else if (tipoFuente === 'fresco') {
                // Semen en fresco - limpiar valores
                semenDniInput.value = '';
                gametoIdInput.value = 'FRESCO_' + Date.now();
            }
        }
        ...
        document.getElementById('formFertilizacion').addEventListener('submit', function(e) {
            ...
            if (accion && accion.value === 'descartar') {
                const motivoDescarte = document.querySelector(
                    `textarea[name="embriones[${ovo.id}][motivo_descarte]"]`);
